...
async function getBestLatestPoint(deviceId, opts = {}) {
  ...
  if (!lastGps) return { error: null, chosen: null, decision: 'no_data' };

  lastGps.source = 'gps';
  return { error: null, chosen: lastGps, decision: 'gps_only' };
}
...
app.post('/api/telemetry', async (req, res) => {
  try {
    ...
    // Bulk Insert GPS only
    if (gpsRows.length === 0) {
      // All points were filtered as duplicates
      return res.json({ status: 'ok', filtered: true });
    }

    const { error: insertError } = await supabase.from('gps_logs').insert(gpsRows);
    if (insertError) {
    ...
    addLog('DATA', `Received ${points.length} pts from ${points[0].device_id}. GPS: ${gpsRows.length}`);
